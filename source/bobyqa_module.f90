...
module bobyqa_module
    ...
    abstract interface
        subroutine func (n, x, f)  !! calfun interface
			...
			implicit none
			integer,intent(in)               :: n
            real(mcp),dimension(:),intent(in) :: x
...
!  have been found in the case ```NPT=N+6``` even with more than 100 variables.
    ...
    subroutine bobyqa (n, npt, x, xl, xu, rhobeg, rhoend, iprint, maxfun, calfun)
        ...
        integer,intent(in)                  :: npt     !! number of interpolation conditions. Its value must be in
                                                       ...
                                                       !! recommended.
        real(mcp),dimension(:),intent(inout) :: x       !! Initial values of the variables must be set in X(1),X(2),...,X(N). They
                                                       !! will be changed to the values that give the least calculated F.
        real(mcp),dimension(:),intent(in)    :: xl      !! lower bounds on x. The construction of quadratic models
                                                       !! requires XL(I) to be strictly less than XU(I) for each I. Further,
                                                       !! the contribution to a model from changes to the I-th variable is
                                                       !! damaged severely by rounding errors if XU(I)-XL(I) is too small.
        real(mcp),dimension(:),intent(in)    :: xu      !! upper bounds on x. The construction of quadratic models
                                                       !! requires XL(I) to be strictly less than XU(I) for each I. Further,
                                                       !! the contribution to a model from changes to the I-th variable is
                                                       !! damaged severely by rounding errors if XU(I)-XL(I) is too small.
        real(mcp),intent(in)                 :: rhobeg  !! RHOBEG must be set to the initial value of a trust region radius.  
        ...
        real(mcp),intent(in)                 :: rhoend  !! RHOEND must be set to the final value of a trust
        ...
        real(mcp),parameter :: zero = 0.0_mcp
...
!
90      if (dsq <= 1.0e-3_mcp*xoptsq) then
            fracsq = 0.25_mcp * xoptsq
            sumpq = zero
            do k = 1, npt
                sumpq = sumpq + pq (k)
                sum = - half * xoptsq
...
430             format (/ 4 x, 'Return from BOBYQA because a trust',&
            ...
            else if (ratio <= 0.7_mcp) then
                delta = max (half*delta, dnorm)
            else
                delta = max (half*delta, dnorm+dnorm)
            end if
            if (delta <= 1.5_mcp*rho) delta = rho
!
...
680     if (rho > rhoend) then
            delta = half * rho
            ratio = rho / rhoend
            if (ratio <= 16.0_mcp) then
                rho = rhoend
            else if (ratio <= 250.0_mcp) then
...
!
        if (npt >= n+np) then
            do k = 2 * np, npt
                iw = (real(k-np, mcp)-half) / real (n, mcp)
                ip = k - np - iw * n
                iq = ip + iw
                if (iq > n) iq = iq - n
                ptsid (k) = real (ip, mcp) + real (iq, mcp) / real (np, mcp) + sfrac
...
!
        ...
        do k = 1, npt
            ...
            if (k == kopt) then
                ...
                iq = real (np, mcp) * ptsid (k) - real (ip*np, mcp)
                if (iq > 0) then
                    iw = 1
                    if (ip == 0) iw = 2
...
!
        ...
        if (denom <= 1.0e-2_mcp*vlmxsq) then
...
260     do kpt = 1, npt
            ...
            pq (kpt) = zero
            ip = ptsid (kpt)
            iq = real (np, mcp) * ptsid (kpt) - real (ip*np, mcp)
            if (ip > 0) then
                xp = ptsaux (1, ip)
                xpt (kpt, ip) = xp
            end if
...
30      stepsq = zero
        ...
        if (gredsq*delsq <= 1.0e-4_mcp*qred*qred) go to 190
!
...
!
        if (stplen < blen) then
            ...
            if (sdec <= 0.01_mcp*qred) go to 190
            beta = gredsq / ggsav
            go to 30
        end if
90      crvmin = zero
...
120     iterc = iterc + 1
        ...
        if (temp <= 1.0e-4_mcp*qred*qred) go to 190
        temp = sqrt (temp)
        do i = 1, n
            if (xbdi(i) == zero) then
...
!
        ...
        isav = 0
        redsav = zero
        iu = 17.0_mcp * angbd + 3.1_mcp
        do i = 1, iu
            angt = angbd * real (i, mcp) / real (iu, mcp)
...
!
        if (isav == 0) go to 190
        if (isav < iu) then
            temp = (rdnext-rdlrev) / (redmax+redmax-rdlrev-rdnext)
            angt = angbd * (real(isav, mcp)+half*temp) / real (iu, mcp)
        end if
...
!
        if (sdec > 0.01_mcp*qred) go to 120
190     dsq = zero
        do i = 1, n
...
!  component of X to be in the interval [-1,1].
    ...
    subroutine bobyqa_test()
        ...
        real(mcp),dimension(2) :: x, xl, xu        
        integer :: i,n,npt
 
        real(mcp),parameter :: twopi  = 8.0_mcp * atan (1.0_mcp)
        real(mcp),parameter :: bdl    = -3.0_mcp
        real(mcp),parameter :: bdu    = 3.0_mcp
        integer,parameter  :: iprint = 2
        integer,parameter  :: maxfun = 500000
        real(mcp),parameter :: rhobeg = 1.0e-2_mcp
        real(mcp),parameter :: rhoend = 1.0e-6_mcp
